...
module "route53" {
  source 		                    = "../../modules/route53/deeplearningtest.org"
  environment                   = var.environment
  app_name                      = var.app_name
...
module "ec2-tools" {
  ...
  owner                         = var.owner
  efs_odoo_id                   = module.efs.efs_odoo_id
  secret_name                   = module.secrets.secrets_backend_name
  vanta_key                     = var.vanta_key
  ec2_ami_id                    = var.ecs_ami_id
  ec2_sg_id                     = module.security-groups.sg_ec2_id
  subnet_id                     = module.vpc.private-a-id
  cidr_block_d                  = var.cidr_block_d
  instance_profile              = module.ec2-role.ecs_instance_profile_name
}
