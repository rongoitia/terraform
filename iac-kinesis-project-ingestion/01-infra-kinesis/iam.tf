...
# POLICY para acceder a data stream
data "aws_iam_policy_document" "kinesis_firehose_access_to_data_stream_assume_policy" {
  statement {
    effect    = "Allow"
    actions   = ["firehose:*","kinesis:*"]
    resources = [aws_kinesis_stream.stream_desde_ms_1.arn]
  }
}

# ROL PARA DELIVERY STREAM
...
# POLICY PARA ACCEDER DESDE DELIVERY A S3
...
# POLICY PARA ACCEDER DESDE DELIVERY A GLUE
...
# POLICY PARA ACCEDER DESDE DELIVERY A DATA STREAM
resource "aws_iam_role_policy" "kinesis_firehose_access_to_data_stream_policy" {
  name   = "kinesis_firehose_access_to_data_strram_policy"
  role   = aws_iam_role.kinesis_firehose_stream_role.name
  policy = data.aws_iam_policy_document.kinesis_firehose_access_to_data_stream_assume_policy.json
}
...
# ASSUME ROL PARA LAMBDA, DESDE DELIVERY
...
# ASSUME ROL PARA LAMBDA, HACIA CLOUDWATCH
