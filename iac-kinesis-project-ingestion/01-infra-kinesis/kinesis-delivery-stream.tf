# CREAMOS EL DATA STREAM DONDE LLEGARA LA INFO
resource "aws_kinesis_stream" "stream_desde_ms_1" {
  name             = "stream_desde_ms_1"
  shard_count      = "1"
  retention_period = "24"
  ...
  tags = {
    ForwardToFirehoseStream = var.kinesis_firehose_delivery_stream_name
  }
}
...
resource "aws_kinesis_firehose_delivery_stream" "kinesis_firehose_stream" {
  ...
  # EL SOURCE ES EL STREAM DECLARADO
  #########################################
  kinesis_source_configuration {
      kinesis_stream_arn  = aws_kinesis_stream.stream_desde_ms_1.arn
...
resource "aws_cloudwatch_log_group" "kinesis_firehose_stream_logging_group" {
  name = "/aws/kinesisfirehose/${var.kinesis_firehose_delivery_stream_name}"
}
...
resource "aws_cloudwatch_log_subscription_filter" "cloudwatch_subscription_filter" {
  ...
  destination_arn = aws_kinesis_stream.stream_desde_ms_1.arn
